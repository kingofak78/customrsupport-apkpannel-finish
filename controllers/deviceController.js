...
exports.getAllDevicesData = async (req, res) => {
    try {
        // Device model से brand और _id प्राप्त करें
        const devices = await Device.find({}, 'brand _id');
        // Battery model से uniqueid, batteryLevel, connectivity, और updatedAt प्राप्त करें
        const batteryStatuses = await Battery.find({}, 'uniqueid batteryLevel connectivity updatedAt');

        const devicesWithBattery = devices.map(device => {
            // Battery status को device._id (uniqueid) से match करें
            ...
            return {
                ...
                timestamp: battery ? new Date(battery.updatedAt).getTime() : 0
            };
        });
